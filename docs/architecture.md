...
How GitLumiere is put together, and why. Screen-by-screen state lives in
...
src/                    SvelteKit, SPA mode. One store per screen.
  └── invoke ─────────► src-tauri/           Tauri commands, worker, watcher.
                          └── calls ───────► crates/gitlumiere-core/   git, via gix.
```

Each layer knows nothing about the one above it.

### `crates/gitlumiere-core`
...
| `settings.rs` | GitLumiere's own behaviour toggles, stored beside the repository list |
...
units in `crates/gitlumiere-core/src/graph.rs`; the stylesheet needs the same
...
`crates/gitlumiere-core/src/shell.rs` is the only module in the workspace that
...
executed. `crates/gitlumiere-core/src/record.rs` holds it: a process-wide ring
...
- **Reads are absent, deliberately.** Log walking, refs, diff and status are
  ...
  panel says so — inventing a `git log` GitLumiere never ran would teach the user
