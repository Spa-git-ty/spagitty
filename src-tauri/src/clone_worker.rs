...
//! had is not something GitLord may delete. If it did not, the directory is
...
use gitlord_core::clone::{self, Progress};
use gitlord_core::shell;
use serde::Serialize;
...
pub fn spawn(
    ...
    token: u64,
) -> gitlord_core::Result<CloneWorker> {
    ...
    let handle = std::thread::Builder::new()
        .name(format!("gitlord-clone-{token}"))
        .spawn({
