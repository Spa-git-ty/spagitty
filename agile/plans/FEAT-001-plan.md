...
1. **`crates/spagitty-core`** — all git reading, via `gix`. No Tauri types, no
...
`crates/spagitty-core/src/graph.rs` and `src/lib/metrics.ts`, and the frontend
...
- `crates/spagitty-core/src/{graph,refs,repo,status,error,shell}.rs`
