...
- **`src-tauri/src/recents.rs`** holds the list of paths. That is application
  state — a record of what the user opened — so it belongs in Spagitty's own
...
the path visible and says Spagitty has not touched it.
...
- `crates/spagitty-core/src/repo.rs` — `RepoSummary`, `summary`
