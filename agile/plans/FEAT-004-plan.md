...
session handle meant a `--set-upstream-to` run while GitLord was open stayed
...
but GitLord does not know who you are until Settings does, and a guess would be
...
- `crates/gitlord-core/src/branches.rs` — new
- `crates/gitlord-core/src/shell.rs` — `checkout`, `create_branch`
