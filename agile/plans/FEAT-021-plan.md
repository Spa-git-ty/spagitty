...
visual system; the names are not." That is why Spagitty reads as a white program
...
- Storage: `spagitty.theme` keeps holding the **mode**, so an existing install's
  choice survives; `spagitty.theme.family` is new. Both are read leniently — an
