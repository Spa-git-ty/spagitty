...
`gitlord-core`, whose token the store adopts. Filtering client-side would leave
...
**Rust.** `crates/gitlord-core/src/ops.rs` (new: reset, revert, cherry-pick,
...
outside `localStorage` keys namespaced under `gitlord.graph.*`.
