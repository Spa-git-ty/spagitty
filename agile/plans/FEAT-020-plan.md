...
The honest version of the feature is not a label beside a button. GitLord runs
`git` from exactly one module — `crates/gitlord-core/src/shell.rs`, whose header
declares it is "the **only** place in GitLord that spawns a process" — and
...
GitLord against their own understanding of git.

## Architecture decision

**The record lives in the core, beside the spawn site, and knows nothing about
the UI.** `crates/gitlord-core/src/record.rs` holds a process-wide ring buffer
...
| File | Change |
| --- | --- |
| `crates/gitlord-core/src/record.rs` | New. Buffer, `Executed`, `Outcome`, `redact`, `observe`, `recent`, `clear`. |
| `crates/gitlord-core/src/shell.rs` | All four spawns funnelled through `command` + `finish` / `record_spawn`. |
| `crates/gitlord-core/src/lib.rs` | Registers the module; header states where the record comes from. |
