...
- **`crates/spagitty-core/src/clone.rs`** — the destination a URL and a parent
...
- **`crates/spagitty-core/src/shell.rs`** — gains `clone_start`, which spawns
...
clone inside a directory the user already had is not something Spagitty may
...
- `crates/spagitty-core/src/clone.rs` — new; `shell.rs` gains `clone_start`;
  `lib.rs`
