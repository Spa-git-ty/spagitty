...
One new core module, `crates/gitlord-core/src/conflicts.rs`, and one screen.
...
uses, so GitLord calls the same files binary that `git diff` does, and the pane
...
- `crates/gitlord-core/src/conflicts.rs` — new; `lib.rs` gains the module
- `crates/gitlord-core/src/fixture.rs` — fixtures for add/add, delete/modify
