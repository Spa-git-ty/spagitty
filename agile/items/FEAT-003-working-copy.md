...
GitLord can read history and cannot add to it. The toolbar's primary button says
...
needs. Until it exists, GitLord is a history viewer.
...
- Signing configuration — the commit goes through the `git` binary, so signing
  works if the user has configured it, but GitLord does not configure it here.
