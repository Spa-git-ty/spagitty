...
| `crates/gitlord-core/src/clone.rs` | 99.05% | — | 97.14% | 99.21% |
...
## Rust — `crates/gitlord-core/src/clone.rs`, 18 tests
...
| `a_bare_host_gives_the_host_as_the_name_the_way_git_clone_does` | Not a special case: refusing it would make GitLord stricter than the command line for no visible reason |
...
## Rust — `crates/gitlord-core/src/shell.rs`, 2 tests
...
| `says GitLord never asks for a password itself` | Criterion 5's honest half: credentials come from a helper, or the clone fails with git's message |
