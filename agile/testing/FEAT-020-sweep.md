...
`git` command GitLord has executed this session — the command as spawned, its
...
**Preconditions for everything below unless a ticket says otherwise:** GitLord
...
- **Priority:** P1
- **Preconditions:** A fresh config (no GitLord settings file), repository open.
...
- **Expected:** `git fetch --prune --progress --all` — not `git fetch`. Compare
  against what you would have typed: the extra flags are GitLord's, and the
