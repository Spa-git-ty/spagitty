...
moved and deleted. Keep a terminal open — most tickets are "does GitLord agree
with git, without having opened that repository".

**The config file.** The list lives at `repositories.json` in GitLord's
...
- **Priority:** P1
- **Preconditions:** `repositories.json` moved aside; GitLord restarted.
- **Steps:** Open **All repositories** from the rail.
- **Expected:** No grid. The screen says GitLord has not been shown a
...
- **Steps:**
  ...
  2. In GitLord, open **All repositories** and press **Refresh** three times.
...
- **Expected:** The index mtime is unchanged and no lock file was left behind.
  GitLord may not modify a repository the user is not working in. Acceptance
  criterion 5.
- **Result:**
...
- **Steps:**
  ...
  2. Quit GitLord fully and start it again.
...
- **Steps:**
  ...
  4. Restart GitLord and return to the screen.
...
- **Priority:** P1
- **Steps:** With GitLord closed, replace `repositories.json` with `{` and start
  it. Repeat with `[1, 2, 3]` and with a line of prose.
- **Expected:** GitLord starts every time. The screen shows the empty state
...
- **Priority:** P3
- **Steps:** With GitLord closed, move its whole app-config directory aside and
...
- **Steps:** Read the footer. Then confirm no repository appears on the screen
  that was never opened in GitLord — in particular, check that a git repository
