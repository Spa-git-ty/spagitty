...
| `crates/gitlord-core/src/conflicts.rs` | 94.42% | — | 90.00% | 95.30% |
...
| `a_nul_byte_after_the_sniff_window_is_still_text` | Git's own `FIRST_FEW_BYTES` rule, so GitLord calls the same files binary that `git diff` does |
